...
def init_db():
    ...
    if user_count == 0:
        ...
        # Also create Alexis as admin
        alexis_id = str(uuid.uuid4())
        alexis_hash = hashlib.sha256("nexus2025".encode()).hexdigest()
        conn.execute(
            "INSERT INTO users (id,email,nombre,password_hash,is_admin) VALUES (?,?,?,?,1)",
            (alexis_id, "avieto@ingesoftcompany.com", "Alexis Vieto", alexis_hash)
        )
        conn.commit()
    conn.close()
