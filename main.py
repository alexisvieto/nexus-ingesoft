    ...
    if DATABASE_URL:
        ...
        cur.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS catalog (
                ...
                desc TEXT NOT NULL, fab TEXT, precio REAL DEFAULT 0,
...
def get_catalog(division: str, sistema: Optional[str]=None,
    ...
    conn = get_db(); cur = conn.cursor()
    sql = f"SELECT * FROM catalog WHERE division={PH}"
    args = [division]
...
def add_catalog(item: CatalogItem, user=Depends(get_current_user)):
    ...
    cur.execute(
        f"INSERT INTO catalog (id,division,sistema,desc,fab,precio,unidad,notas) VALUES ({PH},{PH},{PH},{PH},{PH},{PH},{PH},{PH})",
...
def update_catalog(item_id: str, item: CatalogItem, user=Depends(get_current_user)):
    conn = get_db(); cur = conn.cursor()
    cur.execute(
        f"UPDATE catalog SET sistema={PH},desc={PH},fab={PH},precio={PH},unidad={PH},notas={PH} WHERE id={PH}",
...
def bulk_add(items: List[CatalogItem], user=Depends(get_current_user)):
    ...
    for item in items:
        cur.execute(
            f"SELECT id FROM catalog WHERE lower(desc)=lower({PH}) AND lower(fab)=lower({PH}) AND division={PH}",
